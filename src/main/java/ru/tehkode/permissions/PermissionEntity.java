...
public abstract class PermissionEntity {
    ...
    /**
     * Return non-inherited user suffix. This means if an entity don't has own
    ...
    public boolean has(String permission, String world) {
        ...
        if (this.isDebug()) {
            manager.getLogger().log(Level.INFO, "User {0} checked for \"{1}\", {2}", new Object[]{this.getName(), permission, expression == null ? "no permission found" : "\"" + expression + "\" found"});
        }
    ...
    /**
     * Return options for all worlds Common options stored as null (empty
     * string) as world.
     *
